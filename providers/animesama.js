...
var require_extractors = __commonJS({
  "src/extractors/index.js"(exports2, module2) {
    ...
    async function resolveStream2(playerUrl, providerName) {
    ...
    async function resolveStreams(playerUrls, providerName) {
      ...
      for (var i = 0; i < playerUrls.length; i++) {
        var stream = await resolveStream2(playerUrls[i], providerName);
        if (stream)
          results.push(stream);
      }
      return results;
    ...
    module2.exports = { resolveStream: resolveStream2, resolveStreams, extractAndResolve };
  }
});
...
async function getEpisodePlayers(animeUrl, season, episode) {
  const epUrl = season && episode ? `${animeUrl.replace(/\/$/, "")}/saison${season}/ep${episode}/` : animeUrl;
...
async function getStreams(tmdbId, mediaType, season, episode) {
  ...
  try {
    ...
    if (!animeLinks.length) {
      console.log(`[${PROVIDER_NAME}] Aucun anime trouv\xE9 pour: ${name}`);
      return [];
    }
    const { playerUrls, epUrl } = await getEpisodePlayers(animeLinks[0], season, episode);
    console.log(`[${PROVIDER_NAME}] ${playerUrls.length} player(s) trouv\xE9(s)`);
    const streams = [];
    for (const playerUrl of playerUrls) {
      const resolved = await (0, import_extractors.resolveStream)(playerUrl, PROVIDER_NAME);
    ...
    console.log(`[${PROVIDER_NAME}] ${streams.length} stream(s) r\xE9solus`);
    return streams;
  } catch (err) {

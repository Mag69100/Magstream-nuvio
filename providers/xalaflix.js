...
function getStreams(tmdbId, mediaType, season, episode) {
  ...
  return fetch(BASE_URL + '/?s=' + tmdbId, {
    ...
    .then(function(html) {
      var links = [];
      var regex = /href="(https?:\/\/xalaflix\.cv\/[^"]+)"/gi;
      var match;
      ...
      return fetch(targetUrl, {
        ...
        .then(function(pageHtml) {
          ...
          console.log('[Xalaflix] ' + streams.length + ' stream(s)');
          return streams;
        });
    })
    .catch(function(err) {
      console.error('[Xalaflix] Erreur:', err.message);
      return [];
    });
}

...
async function searchContent(tmdbId, mediaType) {
  ...
  const links = [];
  const regex = /href="(https?:\/\/coflix\.rodeo\/[^"]+)"/gi;
  let match;
  while ((match = regex.exec(html)) !== null) {
    const l = match[1];
    const isMovie = mediaType === 'movie' && (l.includes('/film') || l.includes('/movie'));
    const isTv = mediaType === 'tv' && (l.includes('/serie') || l.includes('/saison'));
